                   ...
                   hid_t dxpl_id, void **req)
{
    ...
    switch (args->op_type) {
        ...
        case H5VL_OBJECT_VISIT: {
            ...
            switch (loc_params->type) {
                case (H5VL_OBJECT_BY_SELF): {
                    ...
                    switch (loc_obj->obj_type) {
                        case H5I_FILE:
                        case H5I_GROUP:
                            if (NULL == (iter_object = RV_group_open(loc_obj, loc_params, ".", H5P_DEFAULT,
                                                                     H5P_DEFAULT, NULL)))
                                FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                "can't open object iteration group");

                            parent_object_header = "groups";
                            iter_object_type     = H5I_GROUP;
                            break;

                        case H5I_DATASET:
                            if (NULL == (iter_object = RV_dataset_open(loc_obj, loc_params, ".", H5P_DEFAULT,
                                                                       H5P_DEFAULT, NULL)))
                                FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                "can't open object iteration dataset");

                            parent_object_header = "datasets";
                            iter_object_type     = H5I_DATASET;
                            break;

                        case H5I_DATATYPE:
                            if (NULL == (iter_object = RV_datatype_open(loc_obj, loc_params, ".", H5P_DEFAULT,
                                                                        H5P_DEFAULT, NULL)))
                                FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                "can't open object iteration dataset");

                            parent_object_header = "datatypes";
                        ...
                        case H5I_ATTR: {
                            ...
                            switch (loc_obj->u.attribute.parent_obj_type) {
                                ...
                                case H5I_DATASET:
                                    if (NULL ==
                                        (iter_object = RV_dataset_open(loc_obj, loc_params, ".", H5P_DEFAULT,
                                                                       H5P_DEFAULT, NULL)))
                                        FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                        "can't open object iteration dataset");
                                ...
                                case H5I_DATATYPE:
                                    if (NULL ==
                                        (iter_object = RV_datatype_open(loc_obj, loc_params, ".", H5P_DEFAULT,
                                                                        H5P_DEFAULT, NULL)))
                                        FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                        "can't open object iteration dataset");
                    ...
                    switch (iter_object_type) {
                        ...
                        case H5I_ATTR: {
                            ...
                            switch (loc_obj->u.attribute.parent_obj_type) {
                                ...
                                case H5I_GROUP:
                                    if (NULL ==
                                        (iter_object = RV_group_open(loc_obj, loc_params,
                                                                     loc_params->loc_data.loc_by_name.name,
                                                                     H5P_DEFAULT, H5P_DEFAULT, NULL)))
                                        FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                        "can't open object iteration group");
                                ...
                                case H5I_DATASET:
                                    if (NULL ==
                                        (iter_object = RV_dataset_open(loc_obj, loc_params,
                                                                       loc_params->loc_data.loc_by_name.name,
                                                                       H5P_DEFAULT, H5P_DEFAULT, NULL)))
                                        FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                        "can't open object iteration dataset");
                                ...
                                case H5I_DATATYPE:
                                    if (NULL ==
                                        (iter_object = RV_datatype_open(loc_obj, loc_params,
                                                                        loc_params->loc_data.loc_by_name.name,
                                                                        H5P_DEFAULT, H5P_DEFAULT, NULL)))
                                        FUNC_GOTO_ERROR(H5E_SYM, H5E_CANTOPENOBJ, FAIL,
                                                        "can't open object iteration dataset");
                        ...
                        default:
                            FUNC_GOTO_ERROR(H5E_OBJECT, H5E_BADVALUE, FAIL,
                                            "invalid parent object type supplied for visit");
                            break;
                    } /* end switch for iter_object_type */

                    target_object_name = loc_params->loc_data.loc_by_name.name;
                    break;
            ...
            object_iter_data.iter_obj_parent = (RV_object_t *)iter_object;

            if ((url_len = snprintf(request_url, URL_MAX_LENGTH, "%s/%s/%s%s", base_URL, parent_object_header,
                                    object_iter_data.iter_obj_parent->URI,
                                    (parent_object_header == "groups") ? "/links" : "")) < 0)
                FUNC_GOTO_ERROR(H5E_LINK, H5E_SYSERRSTR, FAIL, "snprintf error");
            ...
            if ((iter_object_id = H5VLwrap_register(iter_object, iter_object_type)) < 0)
                      ...
                      rv_hash_table_t *visited_link_table)
{
    ...
    for (i = 0; i < num_links; i++) {
        ...
        *link_section_end = '\0';
        ...
        /* Store id for object in table */
        switch (table[i].link_info.type) {
            case H5L_TYPE_HARD:
                H5VL_loc_by_idx_t loc_params;
                loc_params.order = H5_ITER_INC;
                loc_params.n     = 0;
                ...
                while (link_id_ptr <= link_section_end) {
                    char curr = *link_id_ptr;
                    recent1   = recent2;
                    recent2   = curr;

                    if (recent1 == 'i' && recent2 == 'd') {
                        break;
                    }
