                     ...
                     hid_t lcpl_id, hid_t lapl_id, hid_t dxpl_id, void **req)
{
     ...
     */
    if (NULL == (url_encoded_link_name = curl_easy_escape(curl, H5_rest_basename(loc_params->loc_data.loc_by_name.name), 0)))
 ...
 */
herr_t
RV_link_copy(void *src_obj, const H5VL_loc_params_t *loc_params1,
 ...
 */
herr_t
RV_link_move(void *src_obj, const H5VL_loc_params_t *loc_params1,
             void *dst_obj, const H5VL_loc_params_t *loc_params2,
             hid_t lcpl_id, hid_t lapl_id, hid_t dxpl_id, void **req)
{
    herr_t ret_value = SUCCEED;
...
RV_link_iter_callback(char *HTTP_response, void *callback_data_in, void *callback_data_out)
{
    ...
    if (H5_INDEX_CRT_ORDER == link_iter_data->index_type) {
         ...
         */
        if (RV_build_link_table(HTTP_response, link_iter_data->is_recursive, H5_rest_cmp_links_by_creation_order_inc,
        ...
        if (is_recursive && (H5L_TYPE_HARD == table[i].link_info.type)) {
            ...
            if (!strcmp(link_collection, "groups")) {
                ...
                if (RV_HASH_TABLE_NULL == rv_hash_table_lookup(visited_link_table, link_id)) {
                     ...
                     */
                    if (NULL == (url_encoded_link_name = curl_easy_escape(curl, H5_rest_basename(YAJL_GET_STRING(link_field_obj)), 0)))
                        FUNC_GOTO_ERROR(H5E_LINK, H5E_CANTENCODE, FAIL, "can't URL-encode link name");
